...
export class Highlighter {
	constructor(private root: string | HTMLElement | HTMLElement[] | NodeList) {}
	...
	private getRootElements(): HTMLElement[] {
		...
		if (this.root instanceof NodeList) {
	...
	private markSingle(
		...
		for (const textNode of collectTextNodes(root, exclude, iframes)) {
			...
			for (let i = matches.length - 1; i >= 0; i--) {
				...
				if (
					filter &&
					!filter(textNode, matchText, totalOffset + count, count)
				) {
					continue;
				}
	...
	private markRegExpSingle(
		...
		const { className, exclude = [], iframes = false, each, filter } = options;
	...
	private wrapMatchesAcross(
		...
		for (let i = matches.length - 1; i >= 0; i--) {
			...
			for (let j = overlapping.length - 1; j >= 0; j--) {
				...
				if (
					filter &&
					!filter(entry.node, matchText, totalOffset + count, count)
				) {
					continue;
				}
